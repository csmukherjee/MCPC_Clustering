...
def metrics_summary(labels,label,res_list,names):
    colors = ['red', 'blue', 'green', 'yellow', 'black', 'purple', 'orange', 'pink', 'brown', 'cyan']
    n=len(names)
    #print(res_list)
    ...
    for i in range(n):
        ...
        for label_new in labels[i]:
            if first:
                first = False
                plt.scatter(round(NMI(label,label_new),3),round(met.purity_score(label,label_new),3),marker='x',color=colors[i%10], label = names[i])
            else:
                plt.scatter(round(NMI(label,label_new),3),round(met.purity_score(label,label_new),3),marker='x',color=colors[i%10])
    ...
    for i in range(n):
        ...
        for label_new in labels[i]:
            if first:
                first = False
                plt.scatter(round(completeness(label,label_new),3),round(homogeneity(label,label_new),3),marker='x',color=colors[i%10], label = names[i])
            else:
                plt.scatter(round(completeness(label,label_new),3),round(homogeneity(label,label_new),3),marker='x',color=colors[i%10])
    ...
    for i in range(n):
        ...
        for idx, label_new in enumerate(labels[i]):
            if first:
                first = False
                plt.scatter(res_list[i][idx],round(NMI(label,label_new),3),marker='x',color=colors[i%10], label = names[i])
            else:
                plt.scatter(res_list[i][idx],round(NMI(label,label_new),3),marker='x',color=colors[i%10])
...
def metrics_summary_general(labels,label,res_list,names):
    colors = ['red', 'blue', 'green', 'yellow', 'black', 'purple', 'orange', 'pink', 'brown', 'cyan']
    n=len(names)
    #print(res_list)
    #flatten res_list into a single list
    ...
    for i in range(n):
        ...
        for k, label_new in enumerate(labels[i]):
            ...
            Homogeneity_list.append(round(homogeneity(label,label_new),3))
            Completeness_list.append(round(completeness(label,label_new),3))
            max_NMI_list[i] = [max_NMI,max_Purity,max_res]
        #print('Max NMI and Purity for',names[i],':\n',max_NMI,'res:',max_res[0],'\n',max_Purity,'res:',max_res[1] )
    
    data = {'NMI':NMI_list,'Purity':Purity_list,'V-score':V_list,'Homogeneity':Homogeneity_list,'Completeness':Completeness_list}
    df = pd.DataFrame(data, index=res_list_flat)
    #print(df)
    #sort max_NMI_List by NMI
    max_NMI_list = dict(sorted(max_NMI_list.items(), key=lambda item: item[1][0][0],reverse=True))
    #loop through max_NMI_list
    for key in max_NMI_list:
        print('Max NMI and Purity for',names[key],':\n',max_NMI_list[key][0],'res:',max_NMI_list[key][2][0],'\n',max_NMI_list[key][1],'res:',max_NMI_list[key][2][1] )
