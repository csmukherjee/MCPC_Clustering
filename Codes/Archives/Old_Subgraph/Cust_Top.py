...
def part_to_label(partition,n): #partition to labels
...
def get_NMI(G, partition, label):
    #Keep the labels of the nodes in subgraph G only
    new_label = []
    for i in G.nodes():
        new_label.append(label[i])
    #change the partition form {[node2,node4], [,...,]} to label form {[0,2,1,2,grp#,...]}
    part_label = part_to_label(partition,G.number_of_nodes())

    nmi = normalized_mutual_info_score(part_label, new_label)
    return nmi
...
def louvain_partitions(
    G, weight="weight", resolution=1, threshold=0.0000001, seed=None, FR_order=False, FR_Recalc=False, FR_type=0, Mod_type=0, exp_base=6,label=[]
):
